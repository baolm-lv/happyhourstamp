...
public interface StampDefinitionRepository extends JpaRepository<StampDefinitionEntity, String>, StampDefinitionRepositoryCustom {
	...
	@Query("SELECT s FROM StampDefinitionEntity s WHERE s.productBinding.productId = ?1 and s.stampOnsaleStartDate <= ?2 and s.stampOnsaleEndDate >= ?2 ")

 ...
 * Current RedisTemplate doesn't catch exception when io is down, which 
 * causes the whole transaction failed.
 * Overwrite RedisTemplate execute functions, catch exception, and give out warning.
 * query DB if io is down.
 */
...
public class HhsRedisTemplate <K, V> extends RedisTemplate<K, V> {
	private static final Logger logger = LoggerFactory.getLogger(HhsRedisTemplate.class);
